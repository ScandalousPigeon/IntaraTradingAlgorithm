...
class Trader:
    ...
    def trade_velvetfruit_and_options(
        ...
        # =====================
        # VEV_4000 + VEV_5400 FLOW
        # used ONLY to adjust thresholds
        # =====================
        ...
        for trade in state.market_trades.get("VEV_5400", []):
            ...
            # Mark 14 buying = bullish
            if buyer == "Mark 14":
                vev5400_flow += qty

            # Mark 38 buying = bearish
            if buyer == "Mark 38":
                vev5400_flow -= qty

            # Mark 1 buying = bearish
            if buyer == "Mark 1":
                vev5400_flow -= qty

            # Mark 22 selling = bearish
            if seller == "Mark 22":
                vev5400_flow -= qty
        ...
        if combined_flow > FLOW_TRIGGER:
            buy_threshold = FLOW_THRESHOLD

        if combined_flow < -FLOW_TRIGGER:
            sell_threshold = -FLOW_THRESHOLD
